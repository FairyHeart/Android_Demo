...
class XmlContentHandler(
    ...
    private val mTimer: DanmakuTimer,
    var mDispDensity: Float,
    var mDispScaleX: Float,
    var mDispScaleY: Float
) :
    DefaultHandler() {

    private val TRUE_STRING = "true"

    lateinit var result: Danmakus
    ...
    override fun characters(ch: CharArray, start: Int, length: Int) {
        if (item != null) {
            ...
            if (item?.type == BaseDanmaku.TYPE_SPECIAL && text.startsWith("[")
                ...
                var endX = beginX
                var endY = beginY
                val alphaArr = textArr[2]?.split("-")?.toTypedArray() ?: return
    ...
    private fun parseFloat(floatStr: String?): Float = floatStr?.toFloat() ?: 0f

    private fun parseInteger(intStr: String?): Int = intStr?.toInt() ?: 0

    private fun parseLong(longStr: String?): Long = longStr?.toLong() ?: 0
